...
int main(int argc, char **argv)
{
  ...
  if (argc != 4) {
    if (me == 0) {
      printf("Syntax: mpirun -np P client_parallel mode N Niter\n");
  ...
  if (strcmp(mode,"file") == 0)
    cs = new CSlib(0,mode,"tmp.couple",&world);
  else if (strcmp(mode,"zmq") == 0)
    cs = new CSlib(0,mode,"localhost:5555",&world);
    ...
    MPI_Comm both = world;
    world = onlyclient;
    cs = new CSlib(0,mode,&both,&world);
